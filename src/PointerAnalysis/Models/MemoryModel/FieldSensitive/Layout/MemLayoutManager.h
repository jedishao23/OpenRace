...
class MemLayoutManager {
 private:
  ...
  // std::unordered_map<std::pair<llvm::Type *, bool>, MemLayout *>
  // memLayoutMap;
  ...
  const MemLayout *getStructLayout(llvm::StructType *T, const llvm::DataLayout &DL, size_t &lOffset, size_t &pOffset,
    ...
    // padding. the memory beyond the last element is used as padding, adjust
    ...
    // assert(structLayout->hasPadding() ? true: pOffset ==
