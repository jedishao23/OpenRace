...
bool pta::isZeroOffsetTypeInRootType(const Type *rootType, const Type *elemType, const DataLayout &DL) {
  ...
  if (stripedElemType != elemType && stripedRootType != rootType) {
    ...
    // they should not contain each others. we do not consider array length and
    // dimension e.g., [10 * struct.A] not equal to [10 * struct.B]
...
void pta::prettyFunctionPrinter(const Function *func, raw_ostream &os) {
...
static bool isCompatibleType(const Type *T1, const Type *T2) {
  ...
  if (T1 == T2) {
    ...
    // before passing to functions return false;p
...
static bool isCompatibleFunctionType(const FunctionType *FT1, const FunctionType *FT2) {
  ...
  if ((FT1->getNumParams() != FT2->getNumParams()) && !FT2->isVarArg()) {
...
void pta::recordCGNode(const llvm::Value *val) {
...
llvm::StructType *pta::getConvertedFlexibleArrayType(const llvm::StructType *ST, llvm::Type *flexibleArrayElem) {
  ...
  // For example
  // %struct.SUdpConnSet = type { i32, i32, i32, i16, i8*, i32, [8 x i8], i8*
  // (%struct.SRecvInfo*)*, [0 x %struct.SUdpConn] }
  // TODO: is it safe to assume that if the last element of the structure is a
  // zero-sized array, then it is a structure type with flexible array element
  // at the end?
  // TODO: there also might be a optional padding at the end of the structure as
  // well
...
bool pta::isCompatibleCall(const llvm::Instruction *indirectCall, const llvm::Function *target) {
  ...
  //        // the resolved target should have the same number of argument as
  //        provided by the callsite if it is not var_args if
  //        (target->getFunctionType()->getNumParams() !=
  //        call->getNumArgOperands()) {
  //            return false;
  //        }
  //    } else {
  //        // the callsite should at list provide the minimal number of
  //        parameters to var_args function if (call->getNumArgOperands() <
  //        target->getFunctionType()->getNumParams()) {
  //            return false;
  //        }
  //    }
  //
  //    return isCompatibleFunctionType(call->getFunctionType(),
